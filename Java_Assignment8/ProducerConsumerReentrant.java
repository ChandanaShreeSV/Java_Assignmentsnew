...
public class ProducerConsumerReentrant {
    ...
    public static void main(String[] args) {
        DataSharing ds = new DataSharing(5); // buffer capacity 5
        ...
        // let them run a while then interrupt for clean exit
        try {
            Thread.sleep(8000);
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    ...
                    // produce at random pace
                    Thread.sleep(300 + rnd.nextInt(500));
                    if (!success) {
                        // if timed out we can retry or log; here continue
                    }
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
                }
